...
type proxyClient struct {
	*socks5ProxyManager
	...
	tunStackRunning bool
	tunStackMtx     sync.Mutex
...
func (pc *proxyClient) Start() {
	...
	for _, ob := range obs {
		...
		if err != nil {
			logging.Error("tun stack is not running: %s", err)
			ob.Open = false
			pc.socks5ProxyManager.UpdateOutbound(&ob)
			continue
		}
...
func (pc *proxyClient) Stop() {
	...
	for _, ob := range obs {
		...
		err := pc.DeleteOutboundRoute(&ob)
		if err != nil {
			logging.Error("proxyClient:Stop delete outbound route error: %s", err)
		}
	}
	err := pc.stopTunStack()
	if err != nil {
		logging.Error("proxyClient:Stop stop tun stack error: %s", err)
	}
}
...
func (pc *proxyClient) CheckPeer(peer string, token types.ID) (string, error) {
	rsp, err := ResourceAuthorize(pc.h, peer, AuthorizeReq{
		Type: AuthorizeTypeProxy,
		Proxy: &AuthorizeProxyInfo{
			Token: token,
		},
	})
	if err != nil {
...
func (pc *proxyClient) startTunStack() error {
	pc.tunStackMtx.Lock()
	defer pc.tunStackMtx.Unlock()
	if pc.tunStackRunning {
		return nil
	}
	...
	err = setTunAddr(tunName, tunLocal, tunRemote)
	if err != nil {
		tunstack.Stop()
		return err
	}
	pc.tunStackRunning = true
	return nil
}

func (pc *proxyClient) stopTunStack() error {
	pc.tunStackMtx.Lock()
	defer pc.tunStackMtx.Unlock()
	if !pc.tunStackRunning {
		return nil
